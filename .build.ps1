...
task Clean { 
	exec { MSBuild16 "$solution_file" /t:Clean /p:Configuration=Release /p:Platform="AnyCPU" /v:quiet } 
...
task Build {
	exec { MSBuild16 "$solution_file" /t:Build /p:Configuration=Release /p:Platform="AnyCPU" /v:minimal /p:OutDir="$build_artifacts_dir\" }
}
